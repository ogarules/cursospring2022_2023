...
public class NumericalConverter {
    ...
    @Autowired
    private List<INumerical> numericals;

    private INumerical numerical;
    private ICurrency currency;

    public String convert(String number, boolean capitalLetters, String lang) {

        this.currency = currencies.stream()
                                  .filter(r -> r.getLanguage().equals(lang))
                                  .findFirst()
                .orElse(null);
        
        this.numerical = numericals.stream()
                                   .filter(r -> r.getLanguage().equals(lang))
                                   .findFirst()
                .orElse(null);
        ...
        if (Pattern.matches("\\d{1,9},\\d{1,2}", number)) {
            ...
            if (Integer.parseInt(num[0]) == 1) {
                letterNumber = literal + " " + currency.getSingularCurrency() + " "
                        + decimalPart;
            } else {
                letterNumber = literal + " " + currency.getPluralCurrency() + " "
                        + decimalPart;
            }
